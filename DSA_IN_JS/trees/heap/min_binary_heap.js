class MinBinaryHeap {
    ...
    /**
     * remove element from min heap and make new element from heap it's root
     * you have to remove the root and find the new root
     * swap the first element with last one in array
     * remove the last element from array
     * now, compare and the first element with it's largest child and replace it. 
     * 12 13 09
     */
    remove() {
    ...
    sinkDown() {
        ...
        while (true) {
            ...
            if (rightIdx < length) {
                ...
                if (
                    // when left child is larger but right child is smaller than element
                    (indexToSwap === null && rightChild < element) ||
                    // when right and left child both are smaller than element
